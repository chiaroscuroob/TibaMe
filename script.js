// ================= CONFIG =================
const API_KEY = "AIzaSyC6Bqdd4k4h_8yTmdGO9S9qWp_rz8DDPv8"; // ★已確認您的 Key 已填入★
...
// 1. 商品上傳 (★修正：加入 stopPropagation 防止迴圈★)
...
// 2. 參考圖上傳 (★修正：加入 stopPropagation 防止迴圈★)
...
function updateUI() {
    ...
    // 參考圖顯示邏輯
    if (state.refImage) {
        els.refPlaceholder.classList.add('hidden'); // 只隱藏 placeholder
...
// ★修正：改用 gemini-1.5-flash 模型 (解決 403 錯誤)★
...
async function generateAll() {
    ...
    try {
        ...
        state.isLoading = false;
        els.errorMsg.innerText = "生成失敗: " + e.message + " (請檢查 Key 或網路狀態)";
