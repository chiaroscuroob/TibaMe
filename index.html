...
<head>
    ...
    <title>AI Product Scene Generator Pro (v7.3)</title>
    ...
    <header class="bg-white border-b border-zinc-200 h-16 flex-none z-30">
        <div class="max-w-[1600px] mx-auto px-6 h-full flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                <span class="text-xs font-medium text-zinc-400 border px-2 py-1 rounded border-zinc-200 bg-zinc-50">v7.3 All-in-One</span>
            </div>
        </div>
    </header>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
            ...
            <div class="lg:col-span-4 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar h-full pb-20">
                ...
                <section class="space-y-2">
                    ...
                    <div id="product-upload-area" class="border border-dashed border-zinc-300 rounded-xl bg-white hover:bg-zinc-50 transition-all cursor-pointer h-32 flex flex-col items-center justify-center gap-2 group relative">
                        ...
                        <div id="product-preview-container" class="absolute inset-0 w-full h-full hidden bg-white rounded-xl overflow-hidden border border-zinc-200">
                            ...
                            <div id="product-analysis-overlay" class="absolute inset-0 bg-white/80 flex items-center justify-center hidden">
                                <div class="flex items-center gap-2 text-zinc-600 text-xs animate-pulse">
                                   <i data-lucide="scan-eye" class="w-4 h-4"></i> 正在辨識商品...
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div id="product-suggestions" class="flex flex-wrap gap-1.5 px-1">
                            <!-- Suggestions will be injected here -->
                        </div>
                    </div>
                </section>

                <!-- Section 2: Background Source -->
                <section class="space-y-3 pt-2 border-t border-zinc-100">
                    ...
                    <div class="flex p-1 bg-zinc-100 rounded-lg">
                        <button id="mode-upload-btn" class="flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-xs font-medium transition-all bg-white text-zinc-900 shadow-sm">
                            <i data-lucide="image-plus" class="w-3 h-3"></i> 自訂圖片上傳
                        </button>
                        <button id="mode-preset-btn" class="flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-xs font-medium transition-all text-zinc-500 hover:text-zinc-700">
                            <i data-lucide="layout" class="w-3 h-3"></i> AI 風格預設
                        </button>
                    </div>
                    ...
                    <div id="upload-mode-content" class="space-y-3">
                        ...
                        <div id="upload-submode-container" class="bg-indigo-50 border border-indigo-100 p-2 rounded-lg hidden">
                            <label class="text-[10px] font-bold text-indigo-900 uppercase tracking-wider mb-1.5 block">此圖片用途 (How to use this image?)</label>
                            ...
                            <p id="submode-desc" class="text-[10px] text-indigo-800/70 mt-1.5 px-1 leading-tight">
                                • 適合空景照片。AI 會保留照片中的桌子/空間，直接把商品放進去。
                            </p>
                        </div>
                    </div>

                    <!-- Content for Preset Mode -->
                    <div id="preset-mode-content" class="grid grid-cols-2 gap-2 hidden">
                        <!-- Presets will be injected by JS -->
                    </div>
                </section>

                <!-- Section 3: Generate & Prompt -->
                <div class="mt-auto space-y-4 pt-4 border-t border-zinc-200">
                    ...
                    <button id="generate-btn" disabled class="w-full h-12 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all shadow-lg transform active:scale-[0.98] bg-zinc-100 text-zinc-400 cursor-not-allowed">
                        ...
                        <span id="generate-btn-text">生成預設場景 + 4視角</span>
                    </button>
            ...
            <div class="lg:col-span-8 flex flex-col gap-6 h-full overflow-y-auto custom-scrollbar pb-6">
                ...
                <div class="bg-zinc-100 rounded-2xl border border-zinc-200 overflow-hidden relative shadow-inner flex flex-col w-full h-auto min-h-[400px] shrink-0 transition-all group">
                    <div class="h-12 bg-white border-b border-zinc-200 flex items-center justify-between px-4 sticky top-0 z-10 shrink-0">
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-bold text-zinc-800 uppercase tracking-wider flex items-center gap-1">
                                <i data-lucide="sparkles" class="w-3 h-3 text-amber-500"></i>
                                <span id="preview-title">AI Preview</span>
                            </span>
                        </div>
                        <div class="flex gap-2">
                            <button id="download-main-btn" class="flex items-center gap-1 px-3 py-1 bg-zinc-900 text-white text-xs rounded hover:bg-zinc-800 hidden">
                                <i data-lucide="download" class="w-3 h-3"></i> 下載
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 relative flex items-center justify-center p-4 bg-zinc-50/50 overflow-hidden min-h-[400px]">
                        ...
                        <div id="preview-empty" class="text-center text-zinc-400 relative z-10">
                            <div class="w-16 h-16 bg-zinc-100 rounded-full flex items-center justify-center mx-auto mb-3 border border-zinc-200">
                                <i data-lucide="monitor" class="w-6 h-6 text-zinc-300"></i>
                            </div>
                            <p class="text-sm font-medium">準備就緒</p>
                            <p class="text-xs mt-1 text-zinc-400">等待指令中...</p>
                        </div>
                        ...
                        <div id="preview-loading" class="flex flex-col items-center justify-center text-zinc-500 gap-3 relative z-10 hidden">
                            ...
                            <div class="text-center">
                                <p class="text-sm font-bold text-zinc-800">正在生成中...</p>
                                <p class="text-xs text-zinc-400 mt-1">AI 正在計算光影與透視</p>
                            </div>
                        </div>
                ...
                <div id="variations-section" class="space-y-3 pb-10 hidden">
                    <h3 class="text-sm font-bold text-zinc-700 flex items-center gap-2 px-1">
                        <i data-lucide="grid" class="w-4 h-4"></i>
                        多視角變體 (Variations)
                    </h3>
                    <div id="variations-grid" class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <script>
        ...
        async function generateAll() {
            ...
            if (state.bgSourceMode === 'upload' && refBase64) {
                if (state.uploadSubMode === 'composite') {
                    mainInstruction = `TASK: High-Fidelity Composite of ${subjectLabel}.\n1. BACKGROUND: Use Image 2 exactly as is.\n2. SUBJECT: Insert ${subjectLabel} into Image 2.\n3. RELIGHTING: Re-light the product to match the lighting direction and color temperature of the background perfectly.\n4. SHADOWS: Generate deep, realistic contact shadows where the product touches the surface. It must look grounded, not floating.\n5. COLOR GRADING: Match the white balance of the product to the background.\n6. NO CROPPING.\n${qualityPrompt}`;
                } else {
                    mainInstruction = `TASK: High-Quality Object Swap.\nGOAL: Replace the central object in Image 2 with ${subjectLabel} from Image 1.\n1. SUBJECT: ${subjectLabel} is the ONLY hero. Render it with sharp details.\n2. ERASE REFERENCE: Completely remove the object currently in Image 2.\n3. INSERT HERO: Place ${subjectLabel} in that spot.\n4. INTEGRATION: Copy the lighting, shadows, and reflections from Image 2 and apply them to ${subjectLabel}.\n5. NEGATIVE PROMPT: Do not include the original object from Image 2.\n6. NO CROPPING.\n${qualityPrompt}`;
                }
            } else {
                mainInstruction = `TASK: Professional Product Photography of ${subjectLabel}.\n1. GENERATE a background that aligns perfectly with the product's angle using style: "${preset.name}".\n2. Description: ${preset.prompt}\n3. LIGHTING: Use studio-quality lighting.\n4. DEPTH: Ensure a realistic depth of field.\n5. NO CROPPING.\n${qualityPrompt}`;
            }
            if (state.customPrompt) mainInstruction += `\nADDITIONAL REQUIREMENT: ${state.customPrompt}`;
            ...
            try {
                ...
                VARIATION_ANGLES.forEach(async (angle) => {
                    ...
                    if (state.bgSourceMode === 'upload' && refBase64) {
                        if (state.uploadSubMode === 'composite') {
                            varPrompt = `TASK: RE-IMAGINE the scene from a NEW ANGLE: ${angle.label}\n1. Source Material: Texture from Image 2.\n2. SUBJECT: ${subjectLabel}. Keep it sharp and detailed.\n3. RE-GEOMETRY: Re-draw background to match ${angle.label}.\n4. ${angle.instruction}\n5. ${qualityPrompt}`;
                        } else {
                            varPrompt = `TASK: Create a product photo of ${subjectLabel} (from Image 1).\nSTRICT INSTRUCTIONS:\n1. HERO: You MUST draw ${subjectLabel}.\n2. FORBIDDEN: DO NOT draw the object found in Image 2.\n3. TEXTURE EXTRACTION: Extract ONLY the ground texture/material from Image 2.\n4. PERSPECTIVE: If Image 2 is side-view but we need Flat Lay, IGNORE Image 2's depth. Just use its texture.\n5. FORCE ANGLE: ${angle.instruction}\n6. ${qualityPrompt}`;
                        }
                    } else {
                        varPrompt = `TASK: Create a product photo of ${subjectLabel} from angle: ${angle.label}\n1. FORCE ANGLE: ${angle.instruction}\n2. Background Style: ${preset.name}\n3. Ensure valid perspective and sharp details.\n4. ${qualityPrompt}`;
                    }
